		  ...
		  u_int fgColorBGR, u_int bgColorBGR)
{
  ...
  u_char row = 0;
  u_char bit = 0x01;
  u_char oc = c - 0x20;  //CHECK THIS OFFSET

  lcd_setArea(rcol, rrow, rcol + 7, rrow + 12); // CHECK - relative to requested col/row
  while (row < 13){
    while (col < 8){
...
void drawString8x12(u_char col, u_char row, char *string,   
		    u_int fgColorBGR, u_int bgColorBGR)   //added by L.B. -TESTING
{
  u_char cols = col;
  while (*string) {
    drawChar8x12(cols, row, *string++, fgColorBGR, bgColorBGR);
    cols += 9;       //LB.:assuming from the function above "+=6" is the space for each character
  }
}
